...
std::pair<Lam*, Ref> counting_for_gpu(std::array<const thorin::Def*, 2> local_idxs, std::array<const thorin::Def*, 2>  group_size_idxs, long unsigned int idx, Ref bound, DefVec acc, Ref exit, const char* name = "for_body") {
    ...
    auto body   = world
                    ...
                    ->set(name);
    auto for_loop = affine::op_for(world, local_idxs[idx], bound, group_size_idxs[idx], acc, body, exit);
    return {body, for_loop};
}
...
Ref LowerMatrixMediumLevel::rewrite_(Ref def) {
    ...
    if (auto map_reduce_ax = match<matrix::map_reduce>(def); map_reduce_ax) {
        ...
        return call;
    } else if (auto map_reduce_ax = match<matrix::parallel_map_reduce>(def); map_reduce_ax) {
        // meta arguments:
        // * n = out-count, (nat)
        // * S = out-dim, (n*nat)
        // * T = out-type (*)
        ...
        // * combination function (mem, acc, inputs) -> (mem, acc)
        // * input matrixes
        auto [mem, zero, comb, inputs, OutputMatrix, local, group_size] = map_reduce_ax->args<7>();
        auto [n, S, T, m, NI, TI, SI]  = map_reduce_ax->callee()->as<App>()->args<7>();
        world.DLOG("map_reduce_ax {} : {}", map_reduce_ax, map_reduce_ax->type());
        world.DLOG("meta variables:");
        world.DLOG("  n = {}", n);
        ...
        world.DLOG("  comb = {} : {}", comb, comb->type());
        world.DLOG("  inputs = {} : {}", inputs, inputs->type());
        world.DLOG("  local_idxs = {}", local);
        world.DLOG("  group_size_idxs = {}", group_size);
        world.DLOG("  OutputMatrix = {}", OutputMatrix);
        ...
        auto local_idxs = local->projs<2>();
        auto group_size_idxs = group_size->projs<2>();
        ...
        for (u64 i = 0; i < m_nat; ++i) {
            ...
            world.DLOG("  matrix {} = {}", i, mat);
            for (u64 j = 0; j < n_input[i]; ++j) {
                // world.VLOG("    dimension {} / {}", j, n_input[i]);
                auto idx     = indices->proj(n_input[i], j);
                auto idx_lit = Lit::isa(idx);
                if (!idx_lit) {
                    world.DLOG("    index {} {} is not a literal", i, j);
        ...
        for (auto idx : out_indices) {
            ...
            auto [body, for_call]       = counting_for_gpu(local_idxs, group_size_idxs, idx, dim, acc, cont, for_name);
            auto [iter, new_acc, yield] = body->vars<3>();
            cont                        = yield;
            raw_iterator[idx]           = iter;
            iterator[idx]               = world.call<core::bitcast>(world.type_idx(dim_nat_def), iter);
